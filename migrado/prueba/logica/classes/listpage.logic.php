...
class ListPage extends RunnerPage 
{
	...
	function ListPage(&$params) 
	{
		...
		{
			global $tables_data, $masterTablesData, $detailsTablesData, $tableEvents;
	...
	function addCommonHtml() 
	{
		...
		if($this->is508) {
			$this->body["begin"].= "<a href=\"#skipdata\" title=\""."Saltar a la tabla"."\" class=\"runner-s508\">"."Saltar a la tabla"."</a>";
			$this->body["begin"].= "<a href=\"#skipmenu\" title=\""."Saltar al menú"."\" class=\"runner-s508\">"."Saltar al menú"."</a>";
			$this->body["begin"].= "<a href=\"#skipsearch\" title=\""."Saltar a la búsqueda"."\" class=\"runner-s508\">"."Saltar a la búsqueda"."</a>";
			$this->body["begin"].= "<a href=\"templates/helpshortcut.htm\" title=\""."Referencia de teclas rápidas"."\" class=\"runner-s508\">"."Referencia de teclas rápidas"."</a>";
		}
	...
	function prepareForResizeColumns()
	{
		if($this->mode!=LIST_AJAX)
		{
			if($this->debugJSMode === true)
				$this->AddJSFile("include/runnerJS/RunnerResizeGrid");
		}
	}
	...
	function beforeProcessEvent() 
	{
		if($this->eventExists("BeforeProcessList"))
			$this->eventsObject->BeforeProcessList($this->conn);
	}
	/**
	...
	{	
		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			$retval = true;
			if($this->eventExists("AfterDelete") || $this->eventExists("BeforeDelete") || $this->audit) 
			{
			...
			if($this->eventExists("BeforeDelete"))
			{
				...
				$retval = $this->eventsObject->BeforeDelete($where, $deleted_values, $tdeleteMessage);
			...
			if(!$lockRecord && @$_REQUEST["a"] == "delete" && $retval) 
			{
				...
				{
					$tdeleteMessage = $this->deleteMessage;
					$this->eventsObject->AfterDelete($where, $deleted_values,$tdeleteMessage);
					$this->deleteMessage = $tdeleteMessage;
				}
			}
		...
		if(count($this->selectedRecs) && $this->eventExists("AfterMassDelete")) 
			$this->eventsObject->AfterMassDelete($this->recordsDeleted);
	}
	/**
	...
	function BeforeShowList() 
	{
		if($this->eventExists("BeforeShowList"))
		{
			...
			$this->eventsObject->BeforeShowList($this->xt, $templatefile);
	...
	function seekPageInRecSet($strSQL) {
		...
		if($this->eventExists("ListQuery"))
			$listarray = $this->eventsObject->ListQuery($this->searchClauseObj,$this->arrFieldForSort,$this->arrHowFieldSort,$this->masterTable,$this->masterKeysReq,null,$this->pageSize, $this->myPage);
		if($listarray!==false)
	...
	function buildSQL() 
	{
		...
		$agregateFields = GetListOfFieldsByExprType(true,$this->tName);
		...
		$searchHavingClause = $this->searchClauseObj->getWhere($agregateFields);
		...
		$strSQL = gSQLWhere_having($this->gsqlHead, $this->gsqlFrom, $this->gsqlWhereExpr, $this->gsqlGroupBy
		...
		{
			$strSQL = gSQLWhere_having($this->gsqlHead,$this->gsqlFrom,$this->gsqlWhereExpr,$this->gsqlGroupBy, $this->gsqlHaving
			...
			if($rowcount!==false)
				...
				$this->numRowsFromSQL = gSQLRowCount_int($this->gsqlHead,$this->gsqlFrom,$this->gsqlWhereExpr,$this->gsqlGroupBy
					, $this->gsqlHaving,$this->strWhereClause, $this->strHavingClause, $this->strSearchCriteria);
		}
			...
			for($i = 0; $i < count($this->allDetailsTablesArr); $i ++) 
			{
				if($this->allDetailsTablesArr[$i]['dispChildCount']|| $this->allDetailsTablesArr[$i]['hideChild']) 
				{
					...
					foreach($this->masterKeysByD[$i] as $idx => $val) {
						...
						$masterWhere.= AddTableWrappers("subQuery_cnt").".".AddFieldWrappers($this->detailKeysByD[$i][$idx])."=".AddTableWrappers($this->origTName).".".AddFieldWrappers($this->masterKeysByD[$i][$idx]);
					}
	...
	function fillGridShowInfo(&$rowInfoArr)
	{
		...
		if(($this->isUseInlineAdd || $this->showAddInPopup) && $this->permis[$this->tName]['add']) 
		{
			...
			for($i = 0; $i < count($this->listFields); $i ++) 
			{
				...
				$record[GoodFieldName($this->listFields[$i]['fName'])."_class"] .= $this->fieldAlignClass($this->listFields[$i]['fName']);
			}
	...
	function beforeProccessRow() 
	{
		if($this->eventExists("ListFetchArray"))
			$data = $this->eventsObject->ListFetchArray($this->recSet);
		else
		...
		while($data) 
		{
			...
			{
				if(! $this->eventsObject->BeforeProcessRowList($data))
				{
					if($this->eventExists("ListFetchArray"))
						$data = $this->eventsObject->ListFetchArray($this->recSet);
					else
	...
	{
		if(GetFieldData($this->tName,$f,"EditFormat","")==FORMAT_LOOKUP_WIZARD)
			return '';
		$format = GetFieldData($this->tName,$f,"ViewFormat","");
		if($format==FORMAT_AUDIO)
			return ' runner-field-audio"';
		if($format==FORMAT_CHECKBOX)
			return ' runner-field-checkbox"';
		if($format==FORMAT_NUMBER || IsNumberType(GetFieldType($f)))
		...
		if(count($this->totalsFields)) 
		{
			...
			for($j = 0; $j < $this->colsOnPage; $j++) 
			{
				...
				if($j == 0) 
				{
					...
					{
						//	show totals
						$total = GetTotals($this->totalsFields[$i]['fName'], $totals[$this->totalsFields[$i]['fName']], $this->totalsFields[$i]['totalsType'], $this->totalsFields[$i]['numRows'], $this->totalsFields[$i]['viewFormat']);
						...
						$record[GoodFieldName($this->totalsFields[$i]['fName'])."_class"].= $this->fieldAlignClass($this->totalsFields[$i]['fName']);
					}
				}
	...
	{			 	
		...
		else if($listFieldInfo['viewFormat'] == FORMAT_AUDIO) {
		...
		else if($listFieldInfo['viewFormat'] == FORMAT_DATABASE_AUDIO) {
			$titleField = GetFieldData($this->tName, $listFieldInfo['fName'], 'audioTitleField', "");
			$title = "";
			if ($titleField){
				$title = htmlspecialchars(GetData($data, $titleField, ViewFormat($titleField, $titleField)));
		...
		else if($listFieldInfo['viewFormat'] == FORMAT_VIDEO) {
			$value = "";
			$fieldIsUrl = GetFieldData($this->tName, $listFieldInfo['fName'], "fieldIsVideoUrl", false);			
			$fileName = GetData($data, $listFieldInfo['fName'], FORMAT_NONE);
			if (strlen($fileName)){
		...
		else if($listFieldInfo['viewFormat'] == FORMAT_DATABASE_VIDEO) {
			if (@$data[$listFieldInfo['fName']] != NULL){			
				$videoId = 'video_'.GoodFieldName(htmlspecialchars($listFieldInfo['fName'])).'_'.$this->recId;	
	...
	function & createListPage($table,$options)
	{
		...
		$allfields =  GetTableData($table, ".listFields", array());
