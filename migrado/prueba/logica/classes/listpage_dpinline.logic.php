...
class ListPage_DPInline extends ListPage_Embed
{
	...
	function showPage($returnJson = true)
	{
		...
		if($this->masterPageType==PAGE_EDIT || $this->masterPageType==PAGE_ADD || $this->masterPageType == PAGE_LIST && $this->mainMasterPageType!=PAGE_VIEW){
			if (GetTableData($this->tName, ".edit", false) ||  GetTableData($this->tName, ".isUseInlineEdit", false) || GetTableData($this->tName, ".delete", false) && $this->masterPageType!=PAGE_ADD) {
				if ($this->permis[$this->tName]['edit'] || $this->permis[$this->tName]['delete']){
					$bricksExcept[] = "recordcontrol";
				}
			}

			if (GetTableData($this->tName, ".isUseInlineAdd", false) && $this->permis[$this->tName]['add']){
		...
		if (!$returnJson){
			echo'<br>
				<div id="dpShowHide'.$this->id.'" class="dpDiv">
					<img id="dpMinus'.$this->id.'" class="dpImg" border="0" src="images/minus.gif" valign="middle" alt="*" />
					<a name="dt'.$this->id.'" class="dt">'.$this->strCaption.'</a>
				</div>
				<div id="detailPreview'.$this->id.'" class="'.$pageSkinStyle.' runner-pagewrapper dpStyle">'.$contents.'</div>';
			return;
		}
