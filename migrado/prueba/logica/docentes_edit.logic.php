...
$layout = new TLayout("edit2","PurificOrange","MobileOrange");
...
$layout->blocks["top"][] = "details";$page_layouts["docentes_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "docentes_inline_edit.htm" : "docentes_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["dni"]=urldecode(postvalue("editid1"));
$savedKeys["dni"]=urldecode(postvalue("editid1"));
...
//	processing dni - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_dni_".$id);
		$type = postvalue("type_dni_".$id);
		if(FieldSubmitted("dni_".$id))
		{
				$value = prepare_for_db("dni",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "dni"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["dni"] = $value;
...
//	processing dni - end
//	processing apellido - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_apellido_".$id);
		$type = postvalue("type_apellido_".$id);
		if(FieldSubmitted("apellido_".$id))
		{
				$value = prepare_for_db("apellido",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "apellido"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["apellido"] = $value;
		
			}
	
		}
//	processing apellido - end
//	processing nombre - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_nombre_".$id);
		$type = postvalue("type_nombre_".$id);
		if(FieldSubmitted("nombre_".$id))
		{
				$value = prepare_for_db("nombre",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "nombre"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["nombre"] = $value;
		
			}
	
		}
//	processing nombre - end
//	processing identificacion - begin
	$condition = 1;

	if($condition)
	{
		$value = postvalue("value_identificacion_".$id);
		$type = postvalue("type_identificacion_".$id);
		if(FieldSubmitted("identificacion_".$id))
		{
				$value = prepare_for_db("identificacion",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "identificacion"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["identificacion"] = $value;
		
			}
	
		}
//	processing identificacion - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["dni"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: docentes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: docentes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["dni"] = $evalues["dni"];
	$data["apellido"] = $evalues["apellido"];
	$data["nombre"] = $evalues["nombre"];
	$data["identificacion"] = $evalues["identificacion"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("dni"))
		$xt->assign("dni_fieldblock",true);
	else
		$xt->assign("dni_tabfieldblock",true);
	...
	if(!$pageObject->isAppearOnTabs("identificacion"))
		$xt->assign("identificacion_fieldblock",true);
	else
		$xt->assign("identificacion_tabfieldblock",true);
	...
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"dni", "")));
...
//	detail tables
	$showDetailKeys["ausentes"]["masterkey1"] = $data["dni"];		
	$showDetailKeys["justificados"]["masterkey1"] = $data["dni"];		
...
//	dni - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"dni", ""),"field=dni".$keylink,"",MODE_LIST);
	$showValues["dni"] = $value;
	$showFields[] = "dni";
		$showRawValues["dni"] = substr($data["dni"],0,100);

//	apellido - 

		$value = "";
			$value = ProcessLargeText(GetData($data,"apellido", ""),"field=apellido".$keylink,"",MODE_LIST);
	$showValues["apellido"] = $value;
	$showFields[] = "apellido";
		$showRawValues["apellido"] = substr($data["apellido"],0,100);

//	nombre - 

		$value = "";
			$value = ProcessLargeText(GetData($data,"nombre", ""),"field=nombre".$keylink,"",MODE_LIST);
...
//	identificacion - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"identificacion", ""),"field=identificacion".$keylink,"",MODE_LIST);
	$showValues["identificacion"] = $value;
	$showFields[] = "identificacion";
		$showRawValues["identificacion"] = substr($data["identificacion"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='docentes_view.php?".$viewlink."'\"");
