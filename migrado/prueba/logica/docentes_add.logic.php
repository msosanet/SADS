...
add_nocache_headers();
include_once("include/docentes_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["docentes_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "docentes_inline_add.htm";
else
	$templatefile = "docentes_add.htm";
...
//	processing apellido - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["apellido"]=$value;
		}
		}
//	processibng apellido - end
//	processing nombre - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["nombre"]=$value;
		}
		}
//	processibng nombre - end
//	processing identificacion - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["identificacion"]=$value;
		}
		}
//	processibng identificacion - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'docentes_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'docentes_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: docentes_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["dni"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["dni"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["dni"]=@$avalues["dni"];
	$defvalues["apellido"]=@$avalues["apellido"];
	$defvalues["nombre"]=@$avalues["nombre"];
	$defvalues["identificacion"]=@$avalues["identificacion"];
}
