...
function checkTableName($shortTName, $type=false)
{
	...
	if ("docentes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("ausentes" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("tipo_docente" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("justificados" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	return false;
...
{
	$arr=array();
	$strPerm = GetUserPermissions("docentes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="docentes";
	}
	$strPerm = GetUserPermissions("ausentes");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="ausentes";
	}
	$strPerm = GetUserPermissions("tipo_docente");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="tipo_docente";
	}
	$strPerm = GetUserPermissions("justificados");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="justificados";
	}
...
function GetTableURL($table="")
{
	...
	if("ausentes"==$table) 
		return "ausentes";
	if("tipo_docente"==$table) 
		return "tipo_docente";
	if("justificados"==$table) 
		return "justificados";
}
...
{	
	if(!$shortTName)
		return false;
	if("docentes"==$shortTName) 
		return "docentes";
	if("ausentes"==$shortTName) 
		return "ausentes";
	if("tipo_docente"==$shortTName) 
		return "tipo_docente";
	if("justificados"==$shortTName) 
		return "justificados";
}
...
// long binary data?
	...
	{
		$ret="código binario demasiado grande – no puede ser desplegado";
	} 
	else
		$ret = $value;
	if($ret===false)
...
function ProcessLargeText($strValue, $iquery="",$table="", $mode=MODE_LIST, $format = "")
{
	...
	if($format!=EDIT_FORMAT_LOOKUP_WIZARD && $cNumberOfChars>0 && strlen($strValue)>$cNumberOfChars && $mode==MODE_LIST)
	{
		...
		$ret.=' <a href="javascript:void(0);" query="fulltext.php?table='.$table.'&'.$iquery.'">'."Más".' ...</a>';
	}
...
function DisplayLookupWizard($field, $value, $data, $keylink, $mode)
{
	...
	if(Multiselect($field))
	{
		...
		if(strlen($in))
		{
			...
			{
				$lookupArrTmp[] = $lookuprow[0];
			}
			$lookupArr = array_unique($lookupArr);
			$i = 0;
			while($i<sizeof($lookupArr))
			{
				$lookupvalue = $lookupArr[$i];
				if($found)
					$out.= ",";
				$found = true;
...
function gSQLWhere($where, $having="", $searchCriteria="or")
{
	...
	return gSQLWhere_having($sqlHead, $gsqlFrom, $gsqlWhereExpr, $sqlGroupBy, $sqlHaving, $where, $having, $searchCriteria);
}

function gSQLWhere_having($sqlHead, $sqlFrom, $sqlWhere, $sqlGroupBy, $sqlHaving, $where="", $having="", $criteria = "or")
{
...
//	return none if trying to compare numeric field and string value
   ...
   if(!$strQuote && $strSearchOption!="Contains" && $strSearchOption!="Starts with")
	...
	else if(!IsDateFieldType($type) && $strSearchOption!="Contains" && $strSearchOption!="Starts with")
	{
		...
		{
			$sSearchFor=isEnableUpper(db_prepare_string($sSearchFor));
			if($strSearchOption=="Between" && $sSearchFor2)
				$sSearchFor2=isEnableUpper(db_prepare_string($sSearchFor2));
		}
	}
...
*/		
	...
	{		
		...
		if(IsCharType($type) && !$btexttype)
			return $strField." ".$like." ".isEnableUpper(db_prepare_string("%".$sSearchFor."%"));
		else
...
function gSQLRowCount($where,$having="", $searchCriteria="or")
{
	...
	return gSQLRowCount_int($sqlHead,$gsqlFrom,$gsqlWhereExpr,$sqlGroupBy, $sqlHaving,$where,$having,$searchCriteria);
}

function gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlGroupBy, $sqlHaving,$where,$having,$criteria="or")
{
	global $conn;
	...
	if(strlen($sqlGroupBy))
	{
		...
		{
			$countstr = "select count(*) from (".gSQLWhere_having($sqlHead,$sqlFrom,$sqlWhere,$sqlGroupBy, $sqlHaving,$where,$having,$criteria).") a";
		}
		else
		{
			$countstr = gSQLWhere_having($sqlHead,$sqlFrom,$sqlWhere,$sqlGroupBy, $sqlHaving,$where,$having,$criteria);
...
//	default permissions	
	if($table=="tipo_docente")
	{
		return "ADESPI";
	}
//	default permissions	
	if($table=="justificados")
	{
		return "ADESPI";
	}
}
...
// ajax	dependent dropdown
			...
			{	
				...
				echo "&nbsp;<a href=# id=".$openlookup.">"."Seleccionar"."</a>";				
			}
			echo '<input type="hidden" id="'.$cfield.'" name="'.$cfield.'">';
//	add new item link
			if($addnewitem)
			{
				echo "&nbsp;<a href=# id='addnew_".$cfield."'>"."Añadir nuevo"."</a>";
			}
			return;
		}
////////////////////////////////////////////////////////////////////////////////////////////
...
function BuildEditControl($field , $value, $format, $mode, $fieldNum=0, $id="",$validate, $additionalCtrlParams, &$pageObj)
{
	...
	if($format==EDIT_FORMAT_TEXT_FIELD)
	{
		if(IsDateFieldType($type))
		{
			echo '<input id="'.$ctype.'" type="hidden" name="'.$ctype.'" value="date'.EDIT_DATE_SIMPLE.'">'.GetDateEdit($field,$value,0,$fieldNum,$mode,$id,$pageObj);
			
		}
		else
	    {
			if($mode==MODE_SEARCH)
				echo '<input id="'.$cfield.'" '.$inputStyle.' type="text" autocomplete="off" '. (($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD) && $is508==true ? 'alt="'.$strLabel.'" ' : '') . 'name="'.$cfield.'" '.GetEditParams($field).' value="'.htmlspecialchars($value).'">';				
			else
				echo '<input id="'.$cfield.'" '.$inputStyle.' type="text" '.(($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD) && $is508==true ? 'alt="'.$strLabel.'" ' : '').'name="'.$cfield.'" '.GetEditParams($field).' value="'.htmlspecialchars($value).'">';
	...
	else if($format==EDIT_FORMAT_TIME)
	{
		...
		if(count($timeAttrs))
		{
			if($timeAttrs["useTimePicker"])
			{
				...
				echo '<input type="text" '.$inputStyle.' name="'.$cfield.'" '.(($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD) && $is508==true ? 'alt="'.$strLabel.'" ' : '').'id="'.$cfield.'" '.GetEditParams($field).' value="'.htmlspecialchars($tpVal['val']).'">';
				echo '&nbsp;';
	...
	else if($format==EDIT_FORMAT_TEXT_AREA)
	{
		...
		if(UseRTE($field))
		{
			...
			echo " src=\"rte.php?table=".GetTableURL($strTableName)."&"."id=".$id."&".$iquery.$browser."&".($mode==MODE_ADD || $mode==MODE_INLINE_ADD ? "action=add" : '')."\">";  
			echo "</iframe>";
		}
		else{
	...
	{
		echo '<input '.$inputStyle.' id="'.$cfield.'" type="Password" '.(($mode==MODE_INLINE_EDIT || $mode==MODE_INLINE_ADD) && $is508==true ? 'alt="'.$strLabel.'" ' : '').'name="'.$cfield.'" '.GetEditParams($field).' value="'.htmlspecialchars($value).'">';
		
	}
	else if($format==EDIT_FORMAT_DATE)
	{
		echo '<input id="'.$ctype.'" type="hidden" name="'.$ctype.'" value="date'.DateEditType($field).'">'.GetDateEdit($field,$value,DateEditType($field),$fieldNum,$mode,$id, $pageObj);
	}
	else if($format==EDIT_FORMAT_RADIO)
	{
		BuildRadioControl($field,$value,$fieldNum,$id,$mode);
	}
...
//	filename edit
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				...
				{
					$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nombre de archivo".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';					
				}
				else
				{
					$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nombre de archivo".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50" value="'.htmlspecialchars($filename).'">';					
			...
			$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="file0" checked>'."Guardar";
			
			if((strlen($value) || $mode==MODE_INLINE_EDIT) && !IsRequired($field))
			{
				$strtype.='<input id="'.$ctype.'_delete" type="Radio" name="'.$ctype.'" value="file1">'."Borrar artículos seleccionados";
			}
			$strtype.='<input id="'.$ctype.'_update" type="Radio" name="'.$ctype.'" value="file2">'."Actualizar";
		}
		else
		{
//	if Add mode
			...
			if($format==EDIT_FORMAT_DATABASE_FILE && GetFilenameField($field))
			{
				$strfilename='<br><label for="filename_'.$cfieldname.'">'."Nombre de archivo".'</label>&nbsp;&nbsp;<input type="text" '.$inputStyle.' id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="20" maxlength="50">';			
			}
		}
		...
		echo '<input type="Hidden" id="notempty_'.$cfieldname.'" value="'.(strlen($value) ? 1 : 0).'">';
			...
			BuildSelectControl($field, $value, $fieldNum, $mode,$id, $additionalCtrlParams, $pageObj);
...
//	filename edit
			...
			if(UseTimestamp($field))
				$filename_size=50;
			$strfilename='<input type=hidden name="filenameHidden_'.$cfieldname.'" value="'.htmlspecialchars($filename).'"><br>'."Nombre de archivo".'&nbsp;&nbsp;<input type="text" style="background-color:gainsboro" disabled id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="'.$filename_size.'" maxlength="100" value="'.htmlspecialchars($filename).'">';
			if ( $mode==MODE_INLINE_EDIT ) {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked class="runner-uploadtype">'."Guardar";
			} else {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked class="runner-uploadtype">'."Guardar";
			}
			...
			if((strlen($value) || $mode==MODE_INLINE_EDIT) && !IsRequired($field))
			{
				$strtype.='<input id="'.$ctype.'_delete" type="Radio" name="'.$ctype.'" value="upload1" class="runner-uploadtype">'."Borrar artículos seleccionados";
			}
			$strtype.='<input id="'.$ctype.'_update" type="Radio" name="'.$ctype.'" value="upload2" class="runner-uploadtype">'."Actualizar";
		}
		else
		{
//	if Adding record		
			...
			$strfilename='<br>'."Nombre de archivo".'&nbsp;&nbsp;<input type="text" id="filename_'.$cfieldname.'" name="filename_'.$cfieldname.'" size="'.$filename_size.'" maxlength="100">';			
		}
...
//	make SQL string
	...
	{
		$status="ADDED";
		$message=""."El registro fue añadido"."";
		$IsSaved = true;
	} 
	else
		$message="<<< "."El registro fue añadido"." >>>";
