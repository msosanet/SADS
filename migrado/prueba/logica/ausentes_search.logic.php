...
$layout->blocks["top"][] = "search";$page_layouts["ausentes_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "ausentes_search.htm";
...
$params['shortTableName'] = 'ausentes';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("notifico", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("notifico", $strTableName)] = GetTableURL(GetLookupTable("notifico", $strTableName));
	
	$pageObject->fillFieldToolTips("notifico");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("notifico");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "notifico";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	$xt->assignbyref("notifico_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("notifico_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("notifico_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_notifico", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("notifico");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"notifico", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"notifico", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
