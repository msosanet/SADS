...
class XTempl
{
	...
	function XTempl()
	{
		...
		$this->assign_function("alumnos_faltas_Chart_chart","xt_showchart",array("chartname"=>"alumnos_faltas_Chart","table"=>"alumnos_faltas Chart"));
...
//			if(method_exists($var["object"],$method))
	...
	{
		global $page_layouts;
		if (array_key_exists($this->template_file, $page_layouts))
		{	
			foreach($page_layouts[$this->template_file]->containers as $cname=>$container)
			{
				foreach($container as $brick)
...
//	set page style
		...
		for($i = 0; $i < count($this->cssFiles); $i++){
			if(isset($this->cssFiles[$i]["stylepath"]) && $this->cssFiles[$i]["stylepath"] != '')
				$styleRows["data"][] = $this->cssFiles[$i]["stylepath"].".css";
			if(isset($this->cssFiles[$i]["pagestylepath"]) && $this->cssFiles[$i]["pagestylepath"] != '')				
				$pagestyleRows["data"][] = $this->cssFiles[$i]["pagestylepath"].".css";
		}
		for($i = 0; $i < count($this->IEcssFiles); $i++)
			$IEcssRows["data"][] = $this->IEcssFiles[$i]["stylepathIE"].".css";
		...
		foreach($layout->containers as $cname=>$container)
		{
			...
			foreach($container as $brick)
			{
				...
				elseif(!array_key_exists($brick["block"], $this->xt_vars))
				{
					continue;
				}
			...
			{
				$styleString = "";
				if(array_key_exists($cname,$layout->skins))
				{
