...
$layout->blocks["top"][] = "search";$page_layouts["alumnos_faltas_Chart_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "alumnos_faltas_Chart_search.htm";
...
$params['shortTableName'] = 'alumnos_faltas_Chart';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("fecha", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("fecha", $strTableName)] = GetTableURL(GetLookupTable("fecha", $strTableName));
	...
	if(isEnableSection508())
		$xt->assign_section("fecha_label","<label for=\"".GetInputElementId("fecha", $id)."\">","</label>");
	else 
	...
	if(GetLookupTable("total", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("total", $strTableName)] = GetTableURL(GetLookupTable("total", $strTableName));
	...
	if(isEnableSection508())
		$xt->assign_section("total_label","<label for=\"".GetInputElementId("total", $id)."\">","</label>");
	else 
	...
	if(GetLookupTable("tipo", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("tipo", $strTableName)] = GetTableURL(GetLookupTable("tipo", $strTableName));
	
	$pageObject->fillFieldToolTips("tipo");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("tipo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("tipo_label","<label for=\"".GetInputElementId("tipo", $id)."\">","</label>");
	else 
		$xt->assign("tipo_label", true);
	
	$xt->assign("tipo_fieldblock", true);		
	$xt->assignbyref("tipo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("tipo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("tipo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_tipo", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("tipo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"tipo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"tipo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
