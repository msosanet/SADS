...
class SearchControl {
	...
	function getCtrlSearchTypeOptions($fName, $selOpt, $not) 
	{
		...
		if ($fType == EDIT_FORMAT_DATE || $fType == EDIT_FORMAT_TIME)
		{
			...
			$options.="<option value=\"More than\" ".(($selOpt=="More than" && !$not)?"selected":"").">"."Más que"."</option>";
			$options.="<option value=\"Less than\" ".(($selOpt=="Less than" && !$not)?"selected":"").">"."Menos que"."</option>";
			$options.="<option value=\"Between\" ".(($selOpt=="Between" && !$not)?"selected":"").">"."Entre"."</option>";
			$options.="<option value=\"Empty\" ".(($selOpt=="Empty" && !$not)?"selected":"").">"."Vacio"."</option>";
		}
	...
	{
		$html = '<img id = "'.$this->getDelButtonId($fName, $recId).'" ctrlId="'.$recId.'" fName="'.GoodFieldName($fName).'" class="searchPanelButton" src="images/search/closeRed.gif" alt="'."Borrar control".'">';
		return $html;
	}
