...
$cicloLect = (isset($_POST['anio'])) ? $_POST['anio'] : $_SESSION['cicloLectivo'];
...
<p style="writing-mode: horizontal-tb; text-orientation: mixed;font-size: 1.2em">Este m&oacute;dulo fue desactivado por falta de mantenimiento. Por favor utilizar el del sitio de<a href="https://docentes.colegiosobral.edu.ar" > calificadores</a></p>
<? /*
	echo "<br><br>";
	...
	echo "<br>";
		...
		$result79 = mysql_query ("SELECT DISTINCT c.anio FROM calificador2 c ORDER BY anio DESC");


			echo "<select name=anio onchange='this.form.submit()'>";
				while ($fila79 = mysql_fetch_array($result79))
				{
					if (isset($_POST['anio']) && $_POST['anio']==$fila79['anio'])
					{echo "<option value=".$fila79['anio']." selected>".$fila79['anio']."</option>";}
					else
					{echo "<option value=".$fila79['anio'].">".$fila79['anio']."</option>";}
	...
	*/?>
		...
		<!-- <input type='submit' value='Ver' name='submitcurso' /></FORM> -->
...
if(isset($_POST["submitcurso"]))
{
	...
	echo "<table border=3 id='customers'>";
		...
		echo "</tr>";
		//print_r($array);
		$sqlalu="SELECT * FROM alumno a,calificador2 c,curso2 cc, cursa cu WHERE cu.alumno=c.dni  AND cu.anio='$cicloLect' AND c.dni=a.dni AND cc.idcurso='$curso' AND cc.idcurso=c.curso AND cc.habilitado='1' AND cc.curso=cu.curso AND cc.division=cu.divi GROUP BY c.dni ORDER BY a.apellido,a.nombre ASC";
		...
		while ($alu = mysql_fetch_array($resultalu))
			...
			echo "<tr>";
			 ...
			 echo "<td bgcolor='$advertencia'><a href='vercalifalumno.php?dni=$alu[dni]&nombre=$nombre&curso=$_POST[curso]&inst=$quenota&anio=$cicloLect' target='_blank' >".$alu['apellido'].", ".$alu['nombre']."   ".$pase."</a></td>";
				$count = count($array);
				for ($i = 0; $i < $count; $i++)
				{$sqlcalif="SELECT * FROM calificador2 c,notas n WHERE c.nota=n.id AND c.dni='$alu[dni]' AND c.curso='$curso' AND c.materia='$array[$i]' AND idnota='$quenota' AND anio='$cicloLect'";
