...
//  $table=new easyTable($pdf, '{45, 17, 28}', $estiloTabla); // Para 1er informe
  $table=new easyTable($pdf, '{35, 10, 10, 10,25}', $estiloTabla); // Para 1er cuatrimestre
...
//   $calif_q = mysql_query("SELECT * FROM calificador WHERE dni = '$alumno[dni]' AND materia = '$materia[idmateria]' AND anio = '$anio'");
   ...
   for ($id = 1; $id<=$instancia['idnota']; $id++)
//   foreach($calif AS $id => $nota)
   ...
   $table->easyCell($calif[3], 'font-style:B; align:C; border:1; valign:M');
   $table->easyCell($_letras[$calif[3]], 'align:C; border:1; valign:M');
