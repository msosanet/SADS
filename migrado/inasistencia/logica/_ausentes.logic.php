...
//			if (strtotime($dia[2])>=strtotime($ini_semana) && strtotime($dia[2])<=strtotime($fin_semana)) {
		...
		} // printf("<!-- %s -->",var_export($dia,true));
	}
	return array($lineas_de_tabla, $lic_largas);
}

function _tabular($ausentes) {
	global $hoy, $ini_semana, $fin_semana;
	$semana = fechasSemana($ini_semana,$fin_semana);
	$fecha = date("Y-m-d",strtotime("-15 day",strtotime($hoy)));
	$lineas_de_tabla = [];
	...
	$lineas_de_tabla[$nL] = $trSinDatos;
	$lic_largas = [];
	$ultReg = [];
	foreach ($ausentes as $dia) {
		if ($dia[1]==$docente && $dia[3]==$articulo) { // repite par docente/articulo
			
			$dia[8] = $dia[8] . "<!-- " . $consecutivos . " -->";
			
			if (strtotime($dia[2]) >= strtotime($ini_semana)) $lineas_de_tabla[$nL] = arma_tr($dia,$lineas_de_tabla[$nL]);
			
			//Actualiza la cantidad de días consecutivos
			if (strtotime($dia[2])==strtotime(siguiente($ult_fecha))) $consecutivos++;
			elseif (strtotime($dia[2])==strtotime($ult_fecha)) {}
			else { $consecutivos = 1; }
			//printf("<p style='text-align:left;background-color: lightyellow'>%s art %s fecha : %s</p><p>Consecutivos: %s</p>",$docente,$articulo,$dia[2],$consecutivos);
			
			//matiene el registro para ser utilizado luego del último de la serie
			$ultReg = $dia;
			$ult_fecha = $dia[2];

		} else { //empieza otro par docente/articulo
			//envía el artículo a otra tabla cuando es por más de 14 días corridos a la fecha 
			if ($consecutivos >= 14) {
				array_pop($lineas_de_tabla);
				$nL--;
				array_push($lic_largas,$ultReg);
			}
			//borra la línea si el último día del artículo no pertenece a la semana mostrada (no hay para mostrar)
			if (strtotime($ultReg[2]) < strtotime($ini_semana)) {
				array_pop($lineas_de_tabla);
				$nL--;
			}
			...
			$lineas_de_tabla[$nL] = $trSinDatos;
			$lineas_de_tabla[$nL] = arma_tr($dia,$lineas_de_tabla[$nL]);
			// printf("<p style='text-align:left;background-color: lightgray'>%s art %s</p>",$docente,$articulo);
		}  // printf("<p>registro BD:</p><pre> %s </pre><p>TR tabla:</p><pre> %s </pre>",var_export($dia,true),var_export($lineas_de_tabla[$nL],true));
	}
	if ($consecutivos >= 14) { //envía el último artículo a la otra tabla cuando es por más de 14 días
		array_pop($lineas_de_tabla);
		$nL--;
		array_push($lic_largas,$ultReg);
	}
...
function arma_tr($dia,$tr) {
	// prepara <tr> para docente, articulo modificando el <td> dia por dia para la semana correspondiente a $hoy, no modifica nada si es otra semana
	global $hoy, $ini_semana, $fin_semana;
	$semana = fechasSemana($ini_semana,$fin_semana);
//	global $semana;
...
function infoVisible($dia){ //Establece qué información muestra y el id para el color de fondo
...
function acomodarClaves($array) { //Elimina las claves no numéricas
	if (is_array($array)) {
		$arreglado = array();
...
function _mostrarld($array) {
	if (is_array($array) && count($array,1)>1) {
		...
		echo "<tr><td>" . $linea[11] . ", " . $linea[10] . " hasta el " . $linea[1] . $linea[3] . $info .  "</td></tr>\n";
		}
		echo "</table>\n";
	}
}
