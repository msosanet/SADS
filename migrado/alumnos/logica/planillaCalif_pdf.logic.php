...
if (isset($_GET['cd']) && preg_match('/[1-7](10|11|[1-9|ab])/i',$_GET['cd'],$coincide)) $curso = $coincide[0];
 ...
 $instancia_q = mysql_query("SELECT DISTINCT idnota FROM `calificador2` WHERE curso = '$curso' AND anio = '$anio' ");
...
//   foreach($calif AS $id => $nota)
   ...
   foreach ($quenotax as $qn)
  {
   if ($qn['idnota']!=$_GET['qn'])
	{$table->easyCell($tipoCalif[$calif[$qn['idnota']]], 'align:C; border:1; valign:M');}
   else
	{$table->easyCell($tipoCalif[$calif[$qn['idnota']]], 'font-style:B; align:C; border:1; valign:M');}   
	
	if ($qn['idnota']!=$ultimo)
//   {$table->easyCell($_letras[$calif[$qn['idnota']]], 'align:C; border:1; valign:M');}
   {$table->easyCell($letrasCalif[$calif[$qn['idnota']]], 'align:C; border:1; valign:M');}
