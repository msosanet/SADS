...
$layout = new TLayout("export","FusionGreen","MobileGreen");
...
$layout->blocks["top"][] = "export";$page_layouts["alumnos_faltas_Report_export"] = $layout;
...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["fecha"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["tipo"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<2)
				continue;
			$keys = array();
			$keys["fecha"] = urldecode($arr[0]);
			$keys["tipo"] = urldecode($arr[1]);
			$selected_recs[] = $keys;
		}
	}
...
$xt->display("alumnos_faltas_Report_export.htm");
