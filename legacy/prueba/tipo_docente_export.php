...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=tipo_docente.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=tipo_docente.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$values = array();
			$values["descripcion"] = GetData($row,"descripcion","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=tipo_docente.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			$format = "";
			$values["descripcion"] = GetData($row,"descripcion",$format);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["descripcion"]).'"';
			echo $outstr;
		}
...
// write header
	...
	echo "</tr>";
			$totals = array();
		$totals["descripcion"] = array("value" => 0, "numRows" => 0);
		$totalsFields[] = array('fName'=>"descripcion", 'totalsType'=>'', 'viewFormat'=>"");
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["descripcion"]);
					else
						echo htmlspecialchars($values["descripcion"]);
			echo '</td>';
			echo "</tr>";
		}
