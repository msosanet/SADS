...
$layout->blocks["top"][] = "details";$page_layouts["ausentes_edit"] = $layout;
...
$templatefile = ($inlineedit == EDIT_INLINE) ? "ausentes_inline_edit.htm" : "ausentes_edit.htm";
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "docente"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["docente"] = $value;
		
			}
	
		}
//	processing docente - end
//	processing fecha_desde - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "fecha_desde"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["fecha_desde"] = $value;
		
			}
	
		}
//	processing fecha_desde - end
//	processing fecha_hasta - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "fecha_hasta"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["fecha_hasta"] = $value;
		
			}
	
		}
//	processing fecha_hasta - end
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "motivo"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["motivo"] = $value;
		
			}
	
		}
//	processing motivo - end
//	processing notifico - begin
	$condition = 1;
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "notifico"=="pass" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["notifico"] = $value;
		
			}
	
		}
//	processing notifico - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: ausentes_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: ausentes_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["docente"] = $evalues["docente"];
	$data["fecha_desde"] = $evalues["fecha_desde"];
	$data["fecha_hasta"] = $evalues["fecha_hasta"];
	$data["motivo"] = $evalues["motivo"];
	$data["notifico"] = $evalues["notifico"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='ausentes_view.php?".$viewlink."'\"");
