			        ...
			        if ($this->fieldsArr[$i]['totalMin']){	
			       	...
			       	if ($this->fieldsArr[$i]['totalAvg']){	
						...
						if($s[$this->fieldsArr[$i]['name']]['count']!=0)
							$s[$this->fieldsArr[$i]['name']]['AVG'] = round($s[$this->fieldsArr[$i]['name']]['AVG']/$s[$this->fieldsArr[$i]['name']]['count'],2);
					}
					if ($this->fieldsArr[$i]['totalSum']){
						if ($this->fieldsArr[$i]['viewFormat'] == "Time")
						{						
							...
							if ($this->fieldsArr[$i]['totalAvg']){
			    	            if(isset($grp['summary'][$this->fieldsArr[$i]['name']]['AVG']))
								{
									...
									if($summary['summary'][$this->fieldsArr[$i]['name']]['count']!=0)
										$summary['summary'][$this->fieldsArr[$i]['name']]['AVG'] = round($summary['summary'][$this->fieldsArr[$i]['name']]['AVG']/$summary['summary'][$this->fieldsArr[$i]['name']]['count'],2);
								}
							}if ($this->fieldsArr[$i]['totalSum']){
...
class Report extends ReportLogic
{
    ...
    function Report($sql, $order, &$searchClauseObj, $connection, $pagesize, $printpagesize, &$params)
    {
    	// copy properties to object
		RunnerApply($this, $params);	
		
		ReportLogic::ReportLogic($sql, $order, $searchClauseObj, $connection, $pagesize, $printpagesize, $params);
	}
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				if(!$this->forExport)
				{
					if ($this->fieldsArr[$i]['showThumb'])
					{
						...
						if(IsUseiBox($this->fieldsArr[$i]['name'], $this->tName))
							...
							$val.= " target=_blank";
						$val.=" href=\"imager.php?table=".GetTableURL($strTableName)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
						$val.= "<img border=0";
						if(isEnableSection508())
							$val.= " alt=\"Image from DB\"";
						$val.=" src=\"imager.php?table=".GetTableURL($strTableName)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['thumbnail']))."&alt=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['repPage'])).$keylink."\">";
						$val.= "</a>";
					}else{
						$val = "<img";
						...
						if(isEnableSection508())
							$val.= " alt=\"Image from DB\"";
						$val.=" src=\"imager.php?table=".GetTableURL($strTableName)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
					}
				}
				else
				{
					$val = "código binario demasiado grande – no puede ser desplegado";
				}
				...
				if(!$this->forExport)
				{
					...
					{
						$val = "<a href=\"getfile.php?table=".GetTableURL($strTableName)."&filename=".rawurlencode($filename)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
			...
			if($gname == $this->repGroupFields[$i]['strGroupField'])
			{
				...
				for($j=0; $j<count($this->fieldsArr); $j++)
				{
						...
						if(is_array($grp['summary'][$this->fieldsArr[$j]['name']]))
						{
							if ($this->fieldsArr[$j]['totalMax'])
							{
								$end["group".GoodFieldName($gname)."_total".$this->fieldsArr[$j]['goodName']."_max"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['MAX'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat'], $this->mode);
							}
							if ($this->fieldsArr[$j]['totalMin']){							
								$end["group".GoodFieldName($gname)."_total".$this->fieldsArr[$j]['goodName']."_min"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['MIN'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat'], $this->mode);
							}
							if ($this->fieldsArr[$j]['totalAvg']){							
								$end["group".GoodFieldName($gname)."_total".$this->fieldsArr[$j]['goodName']."_avg"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['AVG'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat'], $this->mode);
							}
							if ($this->fieldsArr[$j]['totalSum']){		
								$end["group".GoodFieldName($gname)."_total".$this->fieldsArr[$j]['goodName']."_sum"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['SUM'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat'], $this->mode);
							}
						}
					}
					if ($this->fieldsArr[$j]['name'] == $this->repGroupFields[$i]['strGroupField'])
					{
						...
						{
							$formattedValue = getFormattedValue($gvalue, $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat'], $this->mode);
		...
		if ($this->repPageSummary)
		{
	        for($i=0; $i<count($this->fieldsArr); $i++)
	        {
				if(is_array($src[$this->fieldsArr[$i]['name']]))
				{
					if ($this->fieldsArr[$i]['totalSum'])
					{
						$page["page_total".$this->fieldsArr[$i]['goodName']."_sum"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['SUM'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
					}
					if ($this->fieldsArr[$i]['totalAvg']){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_avg"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['AVG'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
					}
					if ($this->fieldsArr[$i]['totalMin']){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_min"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['MIN'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
					}
					if ($this->fieldsArr[$i]['totalMax']){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_max"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['MAX'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
					}
				}
			}
			...
			if(is_array($source["summary"]))
			{
				for($i=0; $i<count($this->fieldsArr); $i++)
				{
					if(is_array($source["summary"][$this->fieldsArr[$i]['name']]))
					{
						if ($this->fieldsArr[$i]['totalMax'])
						{
						    $result["global_total".$this->fieldsArr[$i]['goodName']."_max"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['MAX'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
						}
						if ($this->fieldsArr[$i]['totalMin']){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_min"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['MIN'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
						}
						if ($this->fieldsArr[$i]['totalAvg']){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_avg"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['AVG'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
						}
						if ($this->fieldsArr[$i]['totalSum']){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_sum"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['SUM'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat'], $this->mode);
						}
					}
				}
			}
