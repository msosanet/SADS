	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			foreach($this->groups as $idx => $g)
			{
				...
				if($_SESSION["UserID"] != $data["usuario"] || $g[0] != -1)
				{
			...
			//$row["begin"] = "<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["usuario"])."\">";
			
			$row["username"] = htmlspecialchars($data["usuario"]);
			$row["user"] = $data["usuario"];
			$usercheckbox_attrs.=" name=\"user_".count($this->users)
				."\" id=\"".count($this->users)."\"";
			
			$this->users[] = htmlspecialchars($data["usuario"]);
