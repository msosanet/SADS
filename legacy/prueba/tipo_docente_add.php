...
add_nocache_headers();
include_once("include/tipo_docente_variables.php");
...
$layout->blocks["top"][] = "details";$page_layouts["tipo_docente_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tipo_docente_inline_add.htm";
else
	$templatefile = "tipo_docente_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing descripcion - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$value = postvalue("value_descripcion_".$id);
		$type=postvalue("type_descripcion_".$id);
		if (FieldSubmitted("descripcion_".$id))
		{
				$value=prepare_for_db("descripcion",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "descripcion"=="pass" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["descripcion"]=$value;
		}
		}
//	processibng descripcion - end
//	processing codigo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_codigo_".$id);
		$type=postvalue("type_codigo_".$id);
		if (FieldSubmitted("codigo_".$id))
		{
				$value=prepare_for_db("codigo",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "codigo"=="pass" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["codigo"]=$value;
		}
		}
//	processibng codigo - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'tipo_docente_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'tipo_docente_view.php?'.$keylink.'\'>'."Ver".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tipo_docente_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["descripcion"]=@$avalues["descripcion"];
	$defvalues["codigo"]=@$avalues["codigo"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("descripcion_label","<label for=\"".GetInputElementId("descripcion", $id)."\">","</label>");
...
//	descripcion - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"descripcion", ""),"field=descripcion".$keylink,"",MODE_LIST);
	$showValues["descripcion"] = $value;
	$showFields[] = "descripcion";
		$showRawValues["descripcion"] = substr($data["descripcion"],0,100);
	}	
//	codigo - 
	$display = false;
	if($inlineadd==ADD_MASTER)
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tipo_docente";
		$options['firstTime'] = 1;
