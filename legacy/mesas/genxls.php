...
echo "<table>";
	echo "<tr>";
		echo"<td align='center' bgcolor='#C0C0C0'>Campa馻</td>";
...
for($i=0;$i<count($_SESSION['array']);$i++)
   {
	...
	$fila2 = mysql_fetch_array($result);
			...
			$result8 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='Ingrese su Tel茅fono de Contacto' and sub_id=$fila2[sub_id]");
			...
			$result10 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='Profesi贸n' and sub_id=$fila2[sub_id]");
			...
			$result12 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='Especialidad e Intituci贸n donde realiz贸 la residencia' and sub_id=$fila2[sub_id]");
			$fila13 = mysql_fetch_array($result12);
			$result13 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='Tiene T铆tulo de Especialista?' and sub_id=$fila2[sub_id]");
			$fila14 = mysql_fetch_array($result13);
			$result14 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='Especialidad Certificada e Instituci贸n Certificante' and sub_id=$fila2[sub_id]");
			$fila15 = mysql_fetch_array($result14);
			$result15 = mysql_query ("SELECT * FROM m_salud_cformsdata WHERE field_name='C贸mo conoci贸 esta convocatoria?' and sub_id=$fila2[sub_id]");
