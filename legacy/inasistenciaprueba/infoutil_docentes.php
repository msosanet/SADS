...
<title>INFO UTIL - DOCENTES</title>
...
<p class= "titulo">INFO DOCENTES</p>
<?
$result177 = mysql_query ("SELECT * FROM infoutil WHERE `grupo` = 'docentes' AND ocultar = 0 ORDER BY orden");
?>
<ul>
<?
    while ($fila177 = mysql_fetch_array($result177))
		{
       echo "<li><a href='infoutil_docentes.php?id=" . $fila177[id] . "'>" . $fila177[titulo] . "</a></li>";
}
?>
</ul>
</div>
  ...
  <?   $id = $_GET['id'];
       $result = mysqli_query($mysqli,"SELECT * FROM infoutil WHERE `grupo` = 'docentes' AND id = $id");
