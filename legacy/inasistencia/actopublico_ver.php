...
<table>
    ...
    <tbody>
        ...
        if (!$res) {
            echo "<tr><td colspan='11' style='color:red; font-weight:bold;'>? Error en la consulta: " . mysql_error() . "</td></tr>";
            ...
            while ($row = mysql_fetch_assoc($res)) {
                if ($row['numero'] != $grupo_actual) {
                    ...
                    $contador_grupo++;
                    echo "<tr class='separador'><td colspan='11'>Grupo: {$grupo_actual}</td></tr>";
                }
                ...
                // Mostrar botones solo si el usuario es sebrodalo
                if (isset($_SESSION['usuario']) && $_SESSION['usuario'] == 'sebrodalo') {
                    echo "<td class='btn-acciones'>
                            <a href='editar_acto.php?actor={$row['id']}' style='text-decoration:none;'>
                                <button>Editar</button>
                            </a>
                            <a href='del_acto.php?actor={$row['id']}' style='text-decoration:none;' onclick=\"return confirm('¿Estás seguro de que querés borrar este registro?');\">
